...
import { IonicPage, NavController, NavParams,Platform, MenuController, AlertController, Events, LoadingController, ModalController } from 'ionic-angular';
...
export class AttendancePage {
  ...
  flag12:boolean=false
    ...
    constructor( private storage:Storage,public modalCtrl:ModalController,
       public events:Events,public platform:Platform,public menuCtrl:MenuController,
    ...
    this.service.AttendanceMachine(this.helper.brabch).subscribe(
      (res:any)=>{
        this.Machine_names=res
      },(err:any)=>{
      }
    )
          //       1900   --->     2042
  ...
  // date from
    ...
    search(){
      ...
      console.log(date_from +"  "+date_to)
        ...
        {
          this.service.BindAttendanceGrid(this.machine_id,date_from.toString(),date_to.toString(),this.helper.Emp_ID,this.helper.brabch).subscribe(
            (res:any[])=>{
               this.Attendance_results=res

               for(let i=0;i< this.Attendance_results.length;i++)
               {
                 if(this.Attendance_results[i].Attend_Day!=null){
                  this.Attendance_results[i].Attend_Day= moment(this.Attendance_results[i].Attend_Day).format('L');
                 }else{
                  this.Attendance_results[i].Attend_Day=null
                 }
               }
            },
            (err:any)=>{
