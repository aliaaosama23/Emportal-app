...
import { IonicPage, NavController,Events,Platform , NavParams, MenuController, ModalController, AlertController, LoadingController } from 'ionic-angular';
...
// date to
      ...
      view_all(){
        this.InBox_messages=[]
          this.service.InBox(this.helper.Emp_ID).subscribe(
            (res:any[])=>{
              if(res.length==0){
                this.no_messsages=true
              }else{
                this.InBox_messages=res.reverse()
             //   this.navCtrl.push('AllMessagesPage',this.InBox_messages)
                this.no_messsages=false
                for(let i=0;i< this.InBox_messages.length;i++)
                {
                  this.InBox_messages[i].Msg_Date= moment(this.InBox_messages[i].Msg_Date).format('L');
                }
              }
            },(err:any)=>{
            }
        ...
        this.service.InBox(this.helper.Emp_ID).subscribe(
          (res:any[])=>{
           ...
           //   this.navCtrl.push('AllMessagesPage',this.InBox_messages)
            ...
            this.navCtrl.push('AllMessagesPage',{'all_messages':res})
          },(err:any)=>{
          }
        )
      ...
      open_message(msg_id,Msg_Body){
        this.service.ReadBody(msg_id).subscribe(
          (res:any)=>{
            const alert = this.alertCtrl.create({
              title: this.translate.instant("message_content"),
              subTitle: Msg_Body,
      ...
      search(){
        ...
        console.log(date_from +"  "+date_to)
          ...
          {
            this.InBox_messages=[]
            this.service. SearchInbox(this.helper.Emp_ID,date_from.toString(),date_to.toString(),this. is_read).subscribe(
              (res:any)=>{
                    if(res.length==0){
                      this.no_messsages=true
                    }else{
                      this.InBox_messages=res
                      this.no_messsages=false
                          for(let i=0;i< this.InBox_messages.length;i++)
                          {
                            this.InBox_messages[i].Msg_Date= moment(this.InBox_messages[i].Msg_Date).format('L');
            ...
            this.InBox_messages=[]
              this.translate.get("choose dates").subscribe((val)=>{
                this.service.toast_service(val,3000)
              })
          }
      }
      ...
      download_message(Msg_ID){
       this.service. DownloadFile(Msg_ID,this.helper.Org_ID,this.helper.brabch).subscribe(
         (res:any)=>{
           const fileTransfer:FileTransferObject=this.transfer.create()
           let path=this.file.externalRootDirectory +'Download/' + 'myFile'
    ...
    doRefresh($event){
     ...
     this.service.InBox(this.helper.Emp_ID).subscribe(
       ...
       this.service.OutBox(this.helper.Emp_ID).subscribe(
         (res:any)=>{
          $event.complete()
           this. outBox_messages=res

           for(let i=0;i<this. outBox_messages.length;i++)
             {
               this. outBox_messages[i].Msg_Date= moment( this. outBox_messages[i].Msg_Date).format('L');
             }
         },(err:any)=>{
          $event.complete()
         }
    ...
    async add_message(){
      const modal = await this.modalCtrl.create('AddMessagePage');
      modal.present();
    }
    ...
    OutBox(){
      ...
      loading.present()
        this.service.OutBox(this.helper.Emp_ID).subscribe(
          (res:any[])=>{
            loading.dismiss()
            this. outBox_messages=res.reverse()
